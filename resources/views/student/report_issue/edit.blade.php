...
<div id="form-container-base">
  ...
  <form method="POST" action="{{route('enrolledstudent.student_issue_report.destroy',$message->id)}}">
    @csrf
    @method('DELETE')
   ...
   <div class="form-field" id="LRNyes">
      ...
      <input type="text" name="title" class="@error('title') is-invalid | @enderror" id="title" aria-describedby="title" value="{{$message->title}}" required>
      @error('title')
      <span class="invalid-feedback" role="alert">
        {{$message}}
      </span>
    ...
    <div class="form-field">
      <div class="form-group">
        ...
        <textarea class="form-control" rows="5" id="comment" name="specification" required>{{$message->message}}</textarea>
      </div>
    </div>
    <div class="form-field">

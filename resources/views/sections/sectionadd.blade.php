...
$strandname = DB::table('strands')->where('id',$strand)->first();

@endphp 


    <span id="enrollment-applications-container-base-header">Create a Section in {{$gradename->name}} {{$strandname->name}}<small id="enrolemt-application-updated-at"><i class="fi fi-rr-calendar"></i> Latest update: <?php echo date('m/d/y'); ?></small></span>
...
<input type="hidden" name="strand" value="{{$strandname->name}} ">
    ...
    <div class="form-field">
        ...
        <input placeholder="##" name="lower_gwa" type="number" class="@error('lowergrade_limit') is-invalid |@enderror" id="lowergwa_limit" aria-describedby="lowergwa_limit" value="{{old('lowergwa_limit')}}" maxlength="2" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); " min="75" max="99" onchange="
        ...
        <input placeholder="##" name="upper_gwa" type="number" class="@error('uppergrade_limit') is-invalid |@enderror" id="uppergwa_limit" aria-describedby="uppergwa_limit" value="{{old('uppergwa_limit')}}" maxlength="2" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); " min="75" max="99" onchange="
        if (this.value < 75 || this.value > 99) { 
          this.style.borderColor = 'red'; 
          document.getElementById('ga-alert').style.display = 'block' 
          }
...
$subjects  = DB::table('strands_subjects')->where('strands_id',$strand)->get();
...
<script>
 ...
 <label for="schoolid">Starts at:</label>
        <input placeholder="Section Number" name="starts_at[{{$row }}]" type="time" class="@error('section_number') is-invalid |@enderror" id="guardianfirstname" aria-describedby="starts_at[{{$row }}]" value="{{old('section_number')}}" required>
        @error('starts_at[{{$row }}]')
        <span class="invalid-feedback" role="alert">
          {{$message}}
        </span>
